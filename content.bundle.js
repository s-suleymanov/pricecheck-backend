(() => {
  ...
  function initPriceCheck() {
    ...
    function getAmazonPriceCents() {
      ...
      const root = document.querySelector("#corePriceDisplay_desktop_feature_div") || document.querySelector("#corePriceDisplay_mobile_feature_div") || document.querySelector("#apex_desktop") || document.querySelector("#apex_mobile") || document.querySelector("#ppd") || document.querySelector("#centerCol") || document.body;
      const badText = (t) => {
        const x = String(t || "").toLowerCase();
        return x.includes("list price") || x.includes("typical price") || x.includes("was:") || x.includes("msrp") || x.includes("reference price") || x.includes("with trade-in") || x.includes("/month") || x.includes("/mo") || x.includes("/week") || x.includes("2 weeks") || x.includes("per month") || x.includes("installment") || x.includes("installments") || x.includes("affirm") || x.includes("klarna") || x.includes("or $");
      };
    ...
    const DRIVERS = {
      amazon: {
        ...
        getTitle() {
          return clean(
            document.getElementById("productTitle")?.innerText || document.querySelector("#title span")?.innerText || ""
          );
        },
        getASIN() {
      ...
      target: {
        store: "Target",
        getTitle() {
          const t = document.querySelector('h1[data-test="product-title"]')?.innerText || document.querySelector('meta[property="og:title"]')?.content || document.title;
          return clean(t);
        },
        getPriceCents() {
          const sels = [
      ...
      walmart: {
        ...
        getPriceCents() {
          const el = document.querySelector('[itemprop="price"]') || document.querySelector('[data-automation-id="product-price"]');
          const raw = el?.getAttribute?.("content") || el?.textContent || "";
          return toCents(raw);
        },
        getASIN() {
      ...
      bestbuy: {
        ...
        getTitle() {
          return clean(
            document.querySelector(".sku-title h1")?.innerText || document.querySelector("h1")?.innerText || document.title
          );
        },
        getPriceCents() {
          {
            const m = document.querySelector('meta[property="product:price:amount"]')?.content || document.querySelector('meta[itemprop="price"]')?.content;
            const c = toCents(m);
            if (Number.isFinite(c) && c > 0) return c;
          }
          {
            const el = document.querySelector(".priceView-hero-price span[aria-hidden='true']") || document.querySelector(".priceView-customer-price span") || document.querySelector('[data-testid="customer-price"] span') || document.querySelector('[data-testid="customer-price"]');
            const c = toCents(el?.textContent || "");
            if (Number.isFinite(c) && c > 0) return c;
          }
          {
            const root = document.querySelector('[data-testid="pricing-price"]') || document.querySelector(".priceView-layout") || document.querySelector("#pricing-price") || document.querySelector("main") || document.body;
    ...
    function setRecallAlert(sh, recallUrl) {
      ...
      if (hasRecall) {
        ...
        el.onclick = () => window.open(String(recallUrl).trim(), "_blank", "noopener,noreferrer");
      } else {
        el.style.cursor = "";
        el.onclick = null;
      }
    ...
    const __cache = /* @__PURE__ */ new Map();
    ...
    function slugify(s) {
      const t = clean(String(s || "")).toLowerCase().replace(/&/g, " and ").replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").replace(/-+/g, "-");
      return t || "product";
    }
    ...
    const PS = {
      ...
      lastKey: null,
      isPopulating: false,
      observeMem: /* @__PURE__ */ new Map(),
      // key -> { at, price_cents }
      observeWindow: [],
      observeKey(site, snap) {
        const store = (DRIVERS[site]?.store || "").trim();
        const sku = site === "amazon" ? String(snap?.asin || "").trim().toUpperCase() : String(snap?.store_sku || "").trim();
        if (!store || !sku) return "";
        return `${store}::${sku}`;
      },
      ...
      async ensure() {
        ...
        if (existing) {
          ...
          this.container = this.shadow?.querySelector(".panel")?.parentElement ? this.shadow.querySelector(".panel").closest("div") : this.shadow?.querySelector("div");
          return existing;
        }
      ...
      async populate() {
        ...
        try {
          ...
          const site = siteOf();
          {
            const footer = root.querySelector("#ps-footer") || root.querySelector(".footer") || root.querySelector("#ps-footer-link")?.parentElement;
            if (footer) {
              const hasXOverflow = Math.ceil(document.documentElement.scrollWidth) > Math.ceil(document.documentElement.clientWidth);
          ...
          };
          {
            ...
            const storeSkuForObserve = site === "amazon" ? String(snap.asin || "").trim().toUpperCase() : String(snap.store_sku || "").trim();
          ...
          const recallUrl = Array.isArray(list) ? list.find((r) => nonEmpty(r?.recall_url))?.recall_url || null : null;
